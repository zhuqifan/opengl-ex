...
unsigned char otsu_threshold( int* histogram, int pixel_total )
  ...
  int opening(int width, int height, unsigned char image[],int len)
  {
    erosion(width,height,image,len);
    dilation(width,height,image,len);
    return 0;
  }
  int closing(int width, int height, unsigned char image[],int len)
  {
    dilation(width,height,image,len);
    erosion(width,height,image,len);
    return 0;
...
int main(int argc, char *argv[])
{
  ...
  opening(width*channels,height,image,len);
  write_JPEG_file(argv[2], width, height, channels, image, 100);
  return 0;
}
